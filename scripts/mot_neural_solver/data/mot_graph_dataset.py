    ...
    def _get_last_frame_df(self):
        ...
        for scene in self.seq_names:
            ...
            max_frame = scene_df.frame.max() - (last_graph_frame * scene_step_size)  # Maximum frame at which
                                # 13 -(15*1)                                                     # we can start a graph and still
                                                                                     # have enough frames.

            if scene_df.frame.max() < min_detects: # TODO: min_detects is not updated here when we change it in the tracking_cfg file. Try to set min_detects over there
                min_detects = 3

            #print("\n\n",scene_df['frame_path'][0],"\n\n")
            #print("\n\n",-(min_detects * scene_step_size),"\n\n")
