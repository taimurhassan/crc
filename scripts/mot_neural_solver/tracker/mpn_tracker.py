...
class MPNTracker:
    ...
    def track(self, seq_name, output_path=None, doRefinement = True):
        ...
        print(Path().resolve().stem+"/output/graphs")
        Path(Path().resolve().stem+"/output").mkdir(parents=True, exist_ok=True)
        Path(Path().resolve().stem+"/output/graphs").mkdir(parents=True, exist_ok=True)
        index = 1
        ...
        for start_frame, end_frame in zip(frame_cutpoints[:-1], frame_cutpoints[1:]):
            ...
            #print(seq_name,start_frame,end_frame)
            #print(subseq_graph.graph_obj.x.shape, subseq_graph.graph_obj.edge_index, subseq_graph.graph_obj.edge_preds, subseq_graph.graph_obj.node_names)

            row, col = subseq_graph.graph_obj.edge_index.shape

            if doRefinement:
                index = 0
                for node in subseq_graph.graph_obj.node_names:
                    isPresent = False
                    #print(row,col)
                    for i in range(col):
                        #print(node,subseq_graph.graph_obj.edge_index[0,i])
                        if node == subseq_graph.graph_obj.edge_index[0,i] or node == subseq_graph.graph_obj.edge_index[1,i]:
                            isPresent = True
                            break
                    if isPresent == False:
                        #print("here\n")
                        print("Lone Node:",node) # Here we need to re-assign it to the node with minimum latent feature distance
                        #TODO: For these nodes, measure CNN Feature distances with other nodes and connect them via edge indices
                        nodeFeatures = subseq_graph.graph_obj.x[node,:]
                        #print("Node Features Shape", nodeFeatures.shape)
                        closestNode = -1
                        minDist = 1000000000

                        for k in range(subseq_graph.graph_obj.x.shape[0]):
                            if k != node:
                                features = subseq_graph.graph_obj.x[k,:]
                                sum = 0
                                for l in range(subseq_graph.graph_obj.x.shape[1]):
                                    sum = sum + ((nodeFeatures[l]-features[l]) * (nodeFeatures[l]-features[l]))
                        ...
                        subseq_graph.graph_obj.edge_index[0] += node
                        subseq_graph.graph_obj.edge_index[1] += closestNode

                        subseq_graph.graph_obj.edge_preds += 1

                        #print("Linking Node: ", node, "to its Homogeneous Pair: ", closestNode)

                    index = index + 1
            #input()
            #with open(Path().resolve().stem+'/output/graphs/' + seq_name + '_' + str(index) + '.npy', 'wb') as f:
            with open(Path().resolve().stem+'/output/graphs/' + seq_name + '.npy', 'wb') as f:
                np.save(f, subseq_graph.graph_obj.edge_index)
                np.save(f, subseq_graph.graph_obj.edge_preds)
                np.save(f, subseq_graph.graph_obj.node_names)
                np.save(f, subseq_graph.graph_obj.x)


            index = index + 1

            #with open(Path().resolve().stem+'/output/graphs/'+seq_name+str(start_frame)+'.npy', 'rb') as f:
            #    a = np.load(f)
            #    b = np.load(f)
            #    c = np.load(f)
    ...
    def _add_tracktor_detects(self, seq_df, seq_name):
        ...
        if 'tracktor_id' in big_dets_df:
            print("\n\n",big_dets_df['tracktor_id'],"\n\n")
